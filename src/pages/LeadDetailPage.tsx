...
export default function LeadDetailPage() {
  ...
  async function onGenerate() {
    try {
      ...
      setInfo("Draft generated — review before contacting.");
      setPreview(null);
      load();
    } catch (e) {
      setError(friendlyError(e));
  ...
  return (
    <div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-4">
        <Card>
          ...
          <dl className="mt-2 space-y-1 text-sm">
            <div className="flex justify-between"><dt className="text-neutral-500">Phone</dt><dd>{lead.phone ?? "—"}</dd></div>
            <div className="flex justify-between"><dt className="text-neutral-500">Email</dt><dd>{lead.email ?? "—"}</dd></div>
            <div className="flex justify-between"><dt className="text-neutral-500">Instagram</dt><dd>{lead.instagram ? `@${lead.instagram}` : "—"}</dd></div>
            <div className="flex justify-between"><dt className="text-neutral-500">Website</dt><dd>{lead.website ?? "—"} ({lead.website_status ?? "unknown"})</dd></div>
            <div className="flex justify-between"><dt className="text-neutral-500">Rating</dt><dd>{lead.rating ?? "—"} ({lead.review_count ?? "—"})</dd></div>
          </dl>
          <div className="mt-3 flex flex-wrap gap-1">
            {lead.website && <a href={lead.website.startsWith("http") ? lead.website : `https://${lead.website}`} target="_blank" rel="noreferrer"><GhostButton>Open site</GhostButton></a>}
            {lead.instagram && <a href={`https://instagram.com/${lead.instagram}`} target="_blank" rel="noreferrer"><GhostButton>Open IG</GhostButton></a>}
            {emailHref && <a href={emailHref}><GhostButton>Open email</GhostButton></a>}
          ...
          <div className="mt-3">
            ...
            {lead.follow_up_at && <p className="mt-2 text-xs text-neutral-500">Follow-up: {lead.follow_up_at}</p>}
          </div>
        </Card>

        <Card>
          <div className="text-xs uppercase tracking-wide text-neutral-500">New message</div>
          ...
          <div className="mt-2 flex gap-2">
            <GhostButton onClick={onPreview}>Preview</GhostButton>
      ...
      <h2 className="mt-6 text-sm font-semibold uppercase tracking-wide text-neutral-500">Notes & follow-up</h2>
      <Card>
        <div className="flex gap-2">
          <input value={noteText} onChange={(e) => setNoteText(e.target.value)} placeholder="Add a note…" className="w-full rounded-lg border border-neutral-200 px-3 py-2 text-sm outline-none focus:border-neutral-900" />
        ...
        <div className="mt-2 grid gap-1">
          {notes.map((n) => (
            <div key={n.id} className="text-sm"><span className="text-neutral-500">{n.created_at.slice(0, 10)} · </span>{n.note}{n.follow_up_at && <span className="text-neutral-500"> → follow-up {n.follow_up_at}</span>}</div>
          ))}
          {notes.length === 0 && <p className="text-sm text-neutral-400">No notes yet.</p>}
        </div>
      </Card>

      <h2 className="mt-6 text-sm font-semibold uppercase tracking-wide text-neutral-500">Activity</h2>
      <Card>
        <div className="grid gap-1">
          ...
          {history.length === 0 && <p className="text-sm text-neutral-400">No status changes recorded yet.</p>}
        </div>
      </Card>

      <h2 className="mt-6 text-sm font-semibold uppercase tracking-wide text-neutral-500">Drafts ({msgs.length})</h2>
        ...
        {msgs.map((m) => (
          <Card key={m.id}>
            ...
            <div className="mt-2 flex flex-wrap gap-1">
              <GhostButton onClick={() => copy(`${m.subject ?? ""}\n\n${m.message}`)}>Copy</GhostButton>
              {m.status === "generated" && <GhostButton onClick={() => onStatus(m.id, "approved")}>Approve</GhostButton>}
              {m.status === "approved" && <GhostButton onClick={() => onStatus(m.id, "sent")}>Mark sent</GhostButton>}
              {m.status === "sent" && <GhostButton onClick={() => onStatus(m.id, "replied")}>Mark replied</GhostButton>}
              {!["sent", "replied", "skipped"].includes(m.status) && <GhostButton onClick={() => onStatus(m.id, "skipped")}>Skip</GhostButton>}
            </div>
          </Card>
        ))}
        {msgs.length === 0 && <p className="text-sm text-neutral-400">No drafts yet — preview and generate one.</p>}
      </div>
    </div>
  );
}
